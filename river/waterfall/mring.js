function MRing(groups, options){
	...
	//this.wring = this.wrapRing(this.data);
}
...
MRing.prototype.cookData = function(groups){
	...
	for(var i = 0; i < len1; i++){
		...
		//wrapGroups.push({ angle: angles[i], color: "transparent" });
		var subWingRadius = [];
...
MRing.prototype.mrings = function(groups, data ){//angles, startangles, wingRadius
	var rings = [];
	for(var i = 0, len1 = groups.length; i < len1; i++){
		var sheets = groups[i].groups;
		var subRings = [];
		...
		if(count < total){
			...
			for(var i = 0, len1 = self.rings.length; i < len1; i++){
				...
				for(var j = 0, len2 = subRings.length; j < len2; j++){
					...
					//var r1 = previousR1 + (wingR.r1 - wingR.r2) + radiuGaps[i][j] * count;
				...
				//var trace = self.wring.getTrace(self.data.startangles[i], self.data.angles[i] + angleGaps[i] * count);
				//self.wring.pathes[i].setAttribute("d", trace);
			}
		}else{
			var startangle = self.data.startangles[0];
			for(var i = 0, len1 = self.rings.length; i < len1; i++){
